...
export const roundToStep = (time, step) => {
  const start = time.clone();
  const value = Math.round(start.minute() / step) * step;
  // Re-set the hour to prevent minutes from rounding up the hour
  // Function is not meant for rounding the hour, only minutes
  return time.clone().minutes(value).hour(time.hour());
};
